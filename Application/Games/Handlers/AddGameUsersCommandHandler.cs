...
{
    public class AddGameUsersCommandHandler(
        IDatabase database,
        ILogger<AddGameUsersCommandHandler> logger)
        : ICommandHandler<AddGameUsersCommand, Result<ApplicationGame>>
    {
        public async ValueTask<Result<ApplicationGame>> Handle(AddGameUsersCommand command, CancellationToken cancellationToken)
        {
            var game = await database.Games.FindAsync(command.GameId, cancellationToken);
            if (game is null)
            {
                logger.LogWarning("Game with id {GameId} not found for users addition", command.GameId);
            ...
            {
                logger.LogWarning(
                    "Some users were not found when adding to game with id {GameId}. Missing users: {MissingUserIds}",
                    command.GameId,
            ...
            var alreadyAssociatedUserIds = requestedUserIds.Intersect(currentUserIds).ToArray();
            if (alreadyAssociatedUserIds.Length > 0)
            {
                logger.LogWarning(
                    "Some users are already associated with game id {GameId}: {UserIds}",
                    command.GameId,
                    string.Join(", ", alreadyAssociatedUserIds));
                return Result.Conflict($"The following users are already associated with the game: {string.Join(", ", alreadyAssociatedUserIds)}");
            }

            foreach (var userId in requestedUserIds)
                database.UsersToGames.Add(new UserToGame { GameId = game.Id, UserId = userId });

            await database.SaveChangesAsync(cancellationToken);
            logger.LogInformation("Added users for game with id {GameId}", command.GameId);
